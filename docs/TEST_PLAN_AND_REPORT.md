...
> 下列表格的“待执行”保留为先行测试设计时的基线快照；本轮实际通过、失败、阻塞与未执行清单以第 9.2～9.5 节为准。
...
| INIT-05 | P0 | 首次改密传错误初始密码 | 返回 401，密码不改变 | 待执行 |
| INIT-06 | P0 | 首次改密为合规新密码后重新登录 | 修改成功；旧密码失效；新密码可登录；`mustChangePassword=false` | 待执行 |
| INIT-07 | P1 | 已完成首次改密后再次调用首次改密接口 | 返回状态冲突 | 待执行 |
| INIT-08 | P1 | 修改本人密码，分别测试错误旧密码与正确旧密码 | 错误旧密码失败；正确旧密码成功；旧会话按设计处理 | 待执行 |
| INIT-09 | P1 | 刷新令牌、退出后再刷新 | 登录态可刷新；退出后刷新令牌失效 | 待执行 |
| INIT-10 | P1 | 未携带令牌访问受保护接口 | 返回 401 | 待执行 |

### 7.2 账户、校区与权限

| ID | P | 场景与步骤 | 预期结果 | 状态 |
|---|---|---|---|---|
| ACC-01 | P0 | 管理员创建 QA-A 校区 | 创建成功，代码唯一且启用 | 待执行 |
| ACC-02 | P1 | 重复创建相同校区代码 | 返回重复资源/约束错误，不产生重复数据 | 待执行 |
| ACC-03 | P0 | 管理员创建 M（MINISTER） | 返回一次性初始密码；账号默认启用并要求首次改密 | 待执行 |
| ACC-04 | P0 | 管理员创建 C1（CAMPUS_MANAGER + QA-A） | 创建成功并正确关联 QA-A | 待执行 |
| ACC-05 | P0 | 创建未关联校区的 CAMPUS_MANAGER | 校验失败 | 待执行 |
| ACC-06 | P1 | 创建关联已停用校区的 CAMPUS_MANAGER | 校验失败 | 待执行 |
| ACC-07 | P1 | 用户名非法、过短、重复；邮箱格式错误 | 均被拒绝，错误字段明确 | 待执行 |
| ACC-08 | P0 | M/C1 首次登录并改密 | 均成功；角色、校区和菜单正确 | 待执行 |
| ACC-09 | P0 | M 尝试创建/编辑/停用用户或校区 | 返回 403；页面不显示对应操作入口 | 待执行 |
| ACC-10 | P0 | C1 尝试访问用户列表、审计、通知设置 | 返回 403；页面不显示系统管理入口 | 待执行 |
| ACC-11 | P1 | 管理员重置 C1 密码 | 返回新初始密码；原密码失效；再次强制首次改密 | 待执行 |
| ACC-12 | P1 | 停用 C1 后登录/刷新；再启用 | 停用后不可登录且会话失效；启用后可登录 | 待执行 |
| ACC-13 | P0 | 尝试停用唯一启用管理员 | 操作被拒绝 | 待执行 |
| ACC-14 | P1 | 使用旧 `version` 更新用户/校区 | 返回版本冲突，较新数据不被覆盖 | 待执行 |

### 7.3 项目与需求全流程

| ID | P | 场景与步骤 | 预期结果 | 状态 |
|---|---|---|---|---|
| FLOW-01 | P0 | M 创建 DRAFT 项目并编辑标题/说明 | 创建、编辑成功；创建者和版本正确 | 待执行 |
| FLOW-02 | P0 | 将项目 DRAFT→ACTIVE | 状态和版本更新 | 待执行 |
| FLOW-03 | P1 | 新建项目直接设为 COMPLETED；非法状态跳转 | 均返回状态冲突/校验失败 | 待执行 |
| FLOW-04 | P0 | M 在 ACTIVE 项目创建 QA-A 需求草稿 | 字段、校区、数量、未来截止时间正确 | 待执行 |
| FLOW-05 | P1 | 截止时间为过去、数量 0/10001、空标题 | 校验失败 | 待执行 |
| FLOW-06 | P0 | 编辑草稿并发布 | DRAFT→PUBLISHED；QA-A 负责人收到通知 | 待执行 |
| FLOW-07 | P0 | C1 接受 QA-A 需求 | PUBLISHED→ACCEPTED；参与人出现 C1；创建者收到通知 | 待执行 |
| FLOW-08 | P1 | C1 重复接受同一需求 | 返回重复资源，不产生重复参与人 | 待执行 |
| FLOW-09 | P0 | QA-B 负责人接受 QA-A 需求 | 返回 403，校区隔离有效 | 待执行 |
| FLOW-10 | P1 | C1 在无图片/工时前退出后重新接受 | 退出成功，参与人移除；可再次接受 | 待执行 |
| FLOW-11 | P1 | 已有图片或工时后退出 | 返回状态冲突，业务数据保留 | 待执行 |
| FLOW-12 | P0 | C1 在至少一张可用图片后提交需求 | ACCEPTED→SUBMITTED；创建者收到通知 | 待执行 |
| FLOW-13 | P1 | 无可用图片时提交需求 | 返回状态冲突 | 待执行 |
| FLOW-14 | P0 | M 完成已提交需求 | SUBMITTED→COMPLETED | 待执行 |
| FLOW-15 | P1 | 取消未结束需求并记录原因 | 状态变为 CANCELLED；结束后不可重复取消 | 待执行 |
| FLOW-16 | P0 | 完成项目并尝试新增需求/采用/改需求状态 | 项目变为 COMPLETED；相关业务写操作锁定 | 待执行 |
| FLOW-17 | P0 | M 尝试重新开放项目；管理员执行重新开放 | M 返回 403；管理员可 COMPLETED→ACTIVE | 待执行 |
...
| ID | P | 场景与步骤 | 预期结果 | 状态 |
|---|---|---|---|---|
| OSS-01 | P0 | C1 为已接受需求申请 JPG 上传票据 | 返回图片 ID、PUT URL、对象键；初态 UPLOADING | 待执行 |
| OSS-02 | P0 | 按票据向对象存储 PUT，随后确认上传 | PUT 成功；确认后进入处理并最终 AVAILABLE | 待执行 |
| OSS-03 | P1 | PUT 内容与声明 SHA-256 不一致 | 完成上传时拒绝或处理失败，不产生可用图片 | 待执行 |
| OSS-04 | P1 | 申请 GIF/TXT、空文件、超过 100 MiB | 返回不支持类型/文件过大 | 待执行 |
| OSS-05 | P0 | 非需求参与人申请关联该需求的上传票据 | 返回 403 | 待执行 |
| OSS-06 | P0 | 查询图片详情和图库筛选 | 标题、拍摄者、项目、需求、校区、状态正确 | 待执行 |
| OSS-07 | P1 | C1 编辑本人上传图片元数据；他人越权编辑 | 本人成功；无权用户返回 403 | 待执行 |
| OSS-08 | P0 | 获取原图下载 URL 并下载 | URL 可用，文件哈希/格式正确；审计产生 PHOTO_DOWNLOAD | 待执行 |
| OSS-09 | P1 | 非授权校区负责人查看/下载图片 | 返回 403 或列表不可见 | 待执行 |
| OSS-10 | P0 | M 归档、恢复图片 | AVAILABLE↔ARCHIVED，列表状态同步 | 待执行 |
| OSS-11 | P1 | 删除未采用图片；删除已采用图片 | 未采用可删除；已采用只允许归档 | 待执行 |
...
| ID | P | 场景与步骤 | 预期结果 | 状态 |
|---|---|---|---|---|
| ADOPT-01 | P0 | M 在 ACTIVE 项目采用可用图片 | 采用记录创建；图片视图 `adopted=true`/采用次数更新 | 待执行 |
| ADOPT-02 | P0 | 图库按采用状态查看，打开图片详情 | 列表和详情均显示“已采用”标记 | 待执行 |
| ADOPT-03 | P1 | 同一项目重复采用同一图片 | 返回重复资源，不重复计数 | 待执行 |
| ADOPT-04 | P1 | C1 尝试采用/取消采用 | 返回 403；UI 无采用操作 | 待执行 |
| ADOPT-05 | P1 | 采用非 AVAILABLE、已删除或不存在图片 | 操作失败且整批保持原子性 | 待执行 |
| ADOPT-06 | P0 | 查询采用排行 | `QA2026001` 采用数增加 1 | 待执行 |
| ADOPT-07 | P0 | 取消采用 | 记录标记取消；图片采用标记/计数恢复 | 待执行 |
| ADOPT-08 | P1 | 项目完成后取消采用；取消后重新采用 | 完成后取消被拒绝；重新开放后可按规则操作 | 待执行 |

### 7.6 工时

| ID | P | 场景与步骤 | 预期结果 | 状态 |
|---|---|---|---|---|
| WORK-01 | P0 | C1 为参与需求新增 DRAFT 工时 60+30 分钟 | 创建成功，归属 C1/需求/项目/校区正确 | 待执行 |
| WORK-02 | P1 | 非参与人填报；未来日期；两类分钟均为 0 | 均被拒绝 | 待执行 |
| WORK-03 | P1 | 分钟数 -1/1441；备注超 1000 字 | 校验失败 | 待执行 |
| WORK-04 | P0 | 编辑 DRAFT 后提交 | DRAFT→SUBMITTED；版本递增 | 待执行 |
| WORK-05 | P1 | 已提交工时继续编辑/删除 | 操作被拒绝 | 待执行 |
| WORK-06 | P0 | M 退回工时并填写原因 | SUBMITTED→REJECTED；C1 收到含原因通知 | 待执行 |
| WORK-07 | P0 | C1 将工时改为 75+45 并再次提交 | REJECTED 可编辑；再次变为 SUBMITTED | 待执行 |
| WORK-08 | P0 | M 确认工时 | SUBMITTED→CONFIRMED；C1 收到通知 | 待执行 |
| WORK-09 | P1 | 对 DRAFT/REJECTED 直接确认；重复确认 | 返回状态冲突 | 待执行 |
| WORK-10 | P0 | C1/M/A 分别查询工时列表 | C1 仅见本人；M/A 按权限可见审批范围 | 待执行 |
| WORK-11 | P1 | 使用过期 version 提交/确认/退回 | 返回版本冲突，不覆盖新状态 | 待执行 |
| WORK-12 | P0 | 查询成员统计和总览 | 仅 CONFIRMED 工时计入；分钟合计为 120 | 待执行 |

### 7.7 工时下载与图片批量下载

| ID | P | 场景与步骤 | 预期结果 | 状态 |
|---|---|---|---|---|
| EXPORT-01 | P0 | M 创建成员统计 XLSX 导出任务 | 返回异步任务 ID，最终状态 SUCCEEDED | 待执行 |
| EXPORT-02 | P0 | 轮询任务并下载 XLSX | 下载 URL 有效；文件可打开；表头、C1、75/45/120 分钟正确 | 待执行 |
| EXPORT-03 | P1 | 使用日期、项目、校区条件导出 | 导出行与筛选条件一致 | 待执行 |
| EXPORT-04 | P0 | C1 尝试创建统计导出 | 返回 403，UI 无统计导出入口 | 待执行 |
| EXPORT-05 | P1 | 其他用户读取非本人创建的导出任务 | 返回 403（管理员除外） | 待执行 |
| EXPORT-06 | P0 | M 选择 1 张图片创建 ZIP 下载任务 | 最终 SUCCEEDED；ZIP 可打开且包含正确图片 | 待执行 |
| EXPORT-07 | P1 | 图片批量下载 0 张/201 张 | 校验失败 | 待执行 |
...
| ID | P | 场景与步骤 | 预期结果 | 状态 |
|---|---|---|---|---|
| NOTIFY-01 | P0 | 管理员创建 M/C1 | 新用户各收到账号创建站内通知 | 待执行 |
| NOTIFY-02 | P0 | 发布需求 | QA-A 启用负责人收到通知；其他校区不收到 | 待执行 |
| NOTIFY-03 | P0 | 接受/提交需求 | 需求创建者分别收到通知 | 待执行 |
| NOTIFY-04 | P0 | 工时退回/确认 | 填报人收到对应通知，退回原因正确 | 待执行 |
| NOTIFY-05 | P0 | 查询未读数、单条已读、全部已读 | 未读数正确递减至 0；重复操作幂等 | 待执行 |
| NOTIFY-06 | P1 | 用户读取或标记他人的通知 | 不可见/返回资源不存在或 403 | 待执行 |
| NOTIFY-07 | P0 | 部署级邮件关闭时触发以上事件 | 站内消息正常；不尝试真实邮件；管理端显示不可投递 | 待执行 |
| NOTIFY-08 | P1 | M/C1 访问邮件设置、日志、重试接口 | 返回 403 | 待执行 |
...
| UI-02 | P0 | M 登录后检查菜单并创建项目/需求 | 仅显示部长可用功能；主流程可完成 | 待执行 |
| UI-03 | P0 | C1 登录后检查菜单、接单、上传、填工时 | 仅显示校区负责人功能；主流程可完成 | 待执行 |
| UI-04 | P0 | 通知铃铛查看未读、点击已读/全部已读 | 徽标和列表即时同步 | 待执行 |
| UI-05 | P1 | 刷新各主路由、退出后使用浏览器后退 | 刷新不丢路由；退出后受保护页不可访问 | 待执行 |
| UI-06 | P1 | 1280×720 与常见桌面宽度检查主表格/弹窗 | 无关键按钮遮挡、横向内容可访问 | 待执行 |
| REG-01 | P0 | 执行前端生产构建 | TypeScript 与 Vite 构建通过 | 待执行 |
| REG-02 | P0 | 执行后端自动化测试 | Maven 测试全部通过 | 待执行 |
