...
6. OSS 临时区中的原始图片保留 30 天。系统每天 `00:00` 扫描并删除超过保留期的临时原图，长期保存处理后的图片。
...
- `ZIP`：上传一个不超过 1.5 GB（`1,500,000,000` 字节）的 ZIP 压缩包，包内最多包含 100 张 JPG/PNG。
...
响应返回 `batchId` 和一个或多个 OSS 临时上传凭证。ZIP 解压及图片处理异步执行；服务端必须阻止目录穿越、符号链接及压缩炸弹，不提取非 JPG/PNG 文件。
...
权限：批量上传任务创建人、`ADMIN`。服务端校验 OSS 对象后开始异步解压、校验和压缩。
...
返回批次状态、总数、成功数、失败数及逐文件失败原因。状态为 `UPLOADING`、`PROCESSING`、`PARTIALLY_SUCCEEDED`、`SUCCEEDED` 或 `FAILED`。批次部分失败时保留成功图片，失败图片允许重新上传。
...
1. ZIP 解压后的总大小硬上限；当前建议设为 10 GiB，以防止压缩炸弹。
2. 批量上传时，拍摄者学号、姓名和拍摄时间是整批共用，还是允许逐张填写；当前接口按逐张填写设计。
3. 批量下载数量上限；此前建议单次最多 200 张，尚未确认。
4. 临时原图清理失败后的告警和重试周期；当前建议下次每日任务重试并立即生成管理员告警。
