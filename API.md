...
- 批量生成 photo 前会按 SHA-256 做全库查重（同一批次内的重复文件也算），需求交付和图库一致。重复条目会标记为 `FAILED`，并提示“图片已存在，已跳过重复项”，不会新增重复 photo；批次其余条目照常处理。
- 与单张上传的差异：单张遇到重复时整个请求返回 `DUPLICATE_RESOURCE`；批量只把重复的那一条标为失败。
- 处理失败留下的照片（`UPLOADING` 且带失败原因）不算重复，重传同一个 ZIP 可以把这些图补上。
