...
2. 单张原始图片硬上限为 100 MiB。大于 10 MiB（`10 * 1024 * 1024` 字节）的图片由后端压缩，在不超过 10 MiB 的前提下尽量保留质量。
3. 压缩不改变图片格式：JPEG 通过二分搜索质量参数选择最高可用质量；PNG 先使用无损压缩，仍超过 10 MiB 时再通过二分搜索缩放尺寸。PNG 透明通道必须保留。
...
6. OSS 临时区中的原始图片保留 30 天。系统每天 `00:00` 扫描并删除超过保留期的临时原图，长期保存处理后的图片。单个对象删除失败时不影响其他对象，系统记录失败项、立即生成管理员告警，并在次日 `00:00` 再次尝试。
...
`POST /photos/batch-upload-tickets`

权限：同单图上传。支持以下两种模式：

- `FILES`：一次最多上传 100 张 JPG/PNG，每张不超过 100 MiB。
- `ZIP`：上传一个不超过 1.5 GB（`1,500,000,000` 字节）的 ZIP 压缩包，包内最多包含 100 张 JPG/PNG，解压后文件总大小不得超过 10 GiB（`10 * 1024 * 1024 * 1024` 字节）。
...
`FILES` 模式使用 `files` 数组逐项提供 `fileName`、`contentType`、`size` 和 `sha256`。拍摄者及拍摄时间可在上传完成后逐张补充。

响应返回 `batchId` 和一个或多个 OSS 临时上传凭证。ZIP 解压及图片处理异步执行；服务端必须阻止目录穿越、符号链接及压缩炸弹，不提取非 JPG/PNG 文件。任一安全限制超出时终止整个 ZIP 解压并将批次标记为 `FAILED`。
...
`POST /photos/batches/{batchId}/complete-upload`

权限：批量上传任务创建人、`ADMIN`。服务端校验 OSS 对象后开始异步解压、校验和压缩。逐张元数据尚未填写完整时，处理后的图片保持 `PROCESSING`，不能进入图库。

### 9.5 查询批量上传结果

`GET /photos/batches/{batchId}`

权限：批量上传任务创建人、`ADMIN`。

返回批次状态、总数、成功数、失败数及图片项 `items`。每项包含稳定的 `itemId`、原始文件名、处理状态、元数据是否完整及失败原因。

批次状态为 `UPLOADING`、`PROCESSING`、`WAITING_METADATA`、`PARTIALLY_SUCCEEDED`、`SUCCEEDED` 或 `FAILED`。批次部分失败时保留成功图片，失败图片允许重新上传。

### 9.6 填写批量图片的逐张元数据

`PUT /photos/batches/{batchId}/items/{itemId}/metadata`

权限：批量上传任务创建人、`ADMIN`。每张图片独立填写，拍摄者可以不同。
...
响应返回有效期为 15 分钟的 `downloadUrl`、`expiresAt` 和建议文件名。每次请求应记录下载审计日志。
...
单次最多下载 200 张图片；超过限制返回 `VALIDATION_ERROR`。
...
投递失败不回滚主业务事务，应记录失败原因并异步重试，最多重试 3 次。第三次仍失败时，系统创建管理员告警，并尝试向所有已登记邮箱的启用管理员发送失败通知；告警必须保留在后台，不能仅依赖邮件送达。
...
11. JPG/PNG 原始文件大于 10 MiB 时必须完成后端同格式压缩后才能进入 `AVAILABLE`；单图不得超过 100 MiB。
12. 多负责人参与同一需求时，每条工时仍归属于实际填报人，不在参与人之间自动分摊。
13. 单次批量上传最多 100 张；ZIP 不得超过 1.5 GB，且解压后仍按最多 100 张有效图片校验。
14. ZIP 解压后总大小不得超过 10 GiB；批量上传中的拍摄者学号、姓名及拍摄时间必须逐张填写。
15. 单次批量下载最多 200 张。
...
9. 仅支持 JPG/PNG；单图上限 100 MiB，大于 10 MiB 时由后端保持原格式压缩。
...
14. 临时原图保留 30 天，系统每天 `00:00` 执行清理。
15. 批量上传最多 100 张，支持最大 1.5 GB 的 ZIP；下载签名有效期为 15 分钟。
16. DirectMail 投递最多重试 3 次，仍失败时生成后台告警并尝试通知管理员。
17. ZIP 解压后的文件总大小上限为 10 GiB。
18. 批量上传的拍摄者信息和拍摄时间逐张填写，每张图片可对应不同拍摄者。
19. 单次批量下载最多 200 张。
20. 临时原图清理失败时立即生成管理员告警，并在下一次每日任务中重试。
