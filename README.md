...
| `MFA_ENCRYPTION_KEY` | 两步验证里验证器 App 密钥的加密密钥，Base64 编码的 32 字节随机数（`openssl rand -base64 32`）。不配置就不能绑定验证器 App，也不能打开全站两步验证。**启用后不要更换**：换了之后所有人都要重新绑定；和数据库备份分开保管，但一样要备份 |
| `MFA_WEBAUTHN_RP_ID` | 安全密钥 / 通行密钥绑定的站点域名，例如 `photowarehouse.cn`。生产环境务必配置；留空时按浏览器请求的 Origin 推断，只适合本地开发 |
| `MFA_WEBAUTHN_ORIGINS` | 允许的页面来源，逗号分隔，例如 `https://photowarehouse.cn`；留空时取 `https://<MFA_WEBAUTHN_RP_ID>` |
| `MFA_TRUSTED_DEVICE_TTL` | 登录时"信任此浏览器"的有效期，默认 `30d`，每次凭它登录都顺延 |
| `MFA_STEP_UP_TTL` | 敏感操作验证一次后的信任期，默认 `15m` |
...
管理员在「系统管理 → 两步验证」打开全站开关后生效，关着的时候整套机制不起作用，已绑定的设备会保留。

- **按权限组决定**：「权限管理」里每个权限组可设为不使用 / 建议启用 / 强制启用。系统管理员组固定强制；勾选了「删除、归档和恢复图库图片」「管理需求图片」「新建、编辑、发布和删除选题」「删除需求」中任何一项的权限组也固定强制——这些权限能打开要求验证的删除操作，不强制的话删除就完全不用验证。内置的部长组和校区负责人组默认都持有其中的权限，升级后即为强制。打开全站开关前，操作的管理员必须先在头像菜单的「两步验证」里绑好自己的设备；打开时还要再输一次登录密码、并用设备确认一次。
- **验证方式**：任意验证器 App（Microsoft Authenticator、Google Authenticator 等，扫码绑定，标准 TOTP），或安全密钥 / 通行密钥（浏览器自带的"插入 USB 密钥 / 用手机扫码"窗口，需要 HTTPS 域名）。不需要接入任何第三方平台，也没有短信或邮件费用。
- **什么时候要验证**：已绑定的成员在没有信任过的浏览器上登录；删除图库图片、删除选题、删除需求；进入系统管理面板；批准 MCP 客户端配对。敏感操作验证一次后 15 分钟内不再询问。登录时可以勾选"信任此浏览器"，30 天内在这台浏览器登录不再验证，30 天没有登录自动失效。
- **开启前已登录的会话**：两步验证对某个账号生效后（全站开关打开、所在组改为建议 / 强制、或本人在别的浏览器绑定了设备），这个账号此前签发、没通过第二步的会话会被登出，登录页提示"账号已启用两步验证，请重新登录"。在其中完成绑定或验证的那个会话不受影响。
- **安全提醒**：添加或删除验证设备、以及有人输对密码却连续输错验证码被锁定时，本人会收到站内信（绑定了企业微信的同时收到企业微信消息）。
- **强制组没绑定**：登录后只能进入绑定页，连首次改密都排在绑定之后。建议组没绑定：每次登录后看到一次建议页，可以跳过。
- **丢了手机**：请联系管理员重置密码。重置密码会同时清除该成员的全部验证设备和信任的浏览器，他重新登录后按所在权限组的要求重新绑定。唯一的管理员自己丢了设备时，需要在服务器上直接改库：`UPDATE mfa_setting SET enabled = FALSE WHERE id = 1;`（临时关闭全站开关），登录后重新绑定再打开。
- 验证码输错沿用登录失败的阈值与锁定时长（`LOGIN_MAX_IDENTIFIER_FAILURES` 等），按账号单独计数，重新输入密码不会清零。登录第二步和登录后的再验证分开计数：别人在登录页故意输错，锁不住本人已登录会话里的操作。
...
默认注册 196 个工具（122 个写、74 个读），会占掉宿主不少上下文，有些宿主还有工具数量上限。
