...
因此**必须**在反向代理或网关上为这些路径配置独立的分布式限流。同时注意：审计拦截器会为每一次匿名写操作写入一行 `audit_log`（`operator_id` 为空、`resource_type` 为 `RECRUITMENTS`），请为该表制定保留与清理策略。
...
- 确认网关限流规则已对 `/api/v1/public/recruitments/**` 生效。
