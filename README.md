...
- **按权限组决定**：「权限管理」里每个权限组可设为不使用 / 建议启用 / 强制启用。系统管理员组固定强制，打开全站开关前，操作的管理员必须先在头像菜单的「两步验证」里绑好自己的设备。
...
- **什么时候要验证**：已绑定的成员在没有信任过的浏览器上登录；删除图库图片、删除选题、删除需求；进入系统管理面板。敏感操作验证一次后 15 分钟内不再询问。登录时可以勾选"信任此浏览器"，30 天内在这台浏览器登录不再验证，30 天没有登录自动失效。
...
- 验证码输错与登录失败共用同一套计数与锁定配置（`LOGIN_MAX_IDENTIFIER_FAILURES` 等），按账号单独计数，重新输入密码不会清零。
