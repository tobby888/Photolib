...
输出末尾还会按**当前这套配置**给几条提示：哪些工具因为这次的设置不见了、缺哪个分组会卡住、
想换个样子该改哪个环境变量。提示只说后果，不改任何行为，也不替你决定：

```text
启用分组      : auth, photos, worklogs（3/15 组）
只读模式      : 否
已注册工具    : 31 个

提示（都只是建议，改不改由你）：
  - 开了 photos、worklogs 却没开 directory：上传要 photographer_contact_id、工时要
    member_contact_id，两个都只能从通讯录查，缺了这组这两件事做不完。把 directory 加进
    PHOTOLIB_MCP_TOOLSETS。
  - 当前关掉的分组：projects、requests、adoptions、statistics……要用的时候把它加进
    PHOTOLIB_MCP_TOOLSETS，重启宿主即可，不用重装。
```

分组名写错时 `doctor`（以及服务本身）会打一行人话并以退出码 2 结束，顺带列出所有可选值——
宿主里看到 photolib 起不来，先在终端跑一次 `doctor`。
...
一共 196 个工具（122 个写、74 个读），按分组划分。默认全开——"这个 MCP 要包含项目的所有能力"是它存在的前提；嫌多就按下一节收窄：

| 分组 | 大致内容 |
| --- | --- |
| `auth` | 登录、登录状态、当前身份、两步验证的再验证、改密、系统枚举与限额 |
...
默认全开的 196 个工具会占掉宿主相当一部分上下文，有些宿主还有工具数量上限。**服务本身
...
核对"启用分组 / 只读模式 / 已注册工具 N 个"这三行，末尾的提示会把这套配置的后果说一遍
（少了哪个依赖分组、只读下少了什么）——数字和你预期对不上，多半是宿主还在用旧进程。

### 分组该在什么时候打开

| 分组 | 工具数（只读下剩） | 什么时候打开 | 关掉之后 AI 就做不了 |
| --- | --- | --- | --- |
| `auth` | 7（3） | **基本别关**，见下一节 | 登录、查当前身份、两步验证的再验证、取系统枚举 |
...
- **`auth` 基本不能关。** `photolib_login`、`photolib_whoami`、`photolib_metadata_options`
  ...
  两步验证的再验证 `photolib_step_up` 也在这里：账号启用了两步验证时，删除和系统管理类
  操作会报 `STEP_UP_REQUIRED`，要由用户报出验证器 App 上的 6 位码、调它验证之后才能继续
  （15 分钟内有效；只绑了安全密钥的账号只能去浏览器里操作）。
...
的工具才不会反复去试。全开时 196 个工具会剩 74 个。但有两类东西按这条线一起没了，很容易
踩：
